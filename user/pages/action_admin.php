  ...
  if(!permission("editusers"))
  {
    ...
    // permissions
          ...
          if(!empty($_POST['perm']))
          {
            ...
            db("INSERT INTO ".$db['permissions']." SET `pos` = '0', `user` = '".intval($_GET['user'])."'".$p);
          }
    ////////////////////
    ...
    ////////////////////
        ...
        $del = db("DELETE FROM ".$db['squaduser']." WHERE user = '".intval($_GET['user'])."'");
        
        $del = db("DELETE FROM ".$db['userpos']." WHERE user = '".intval($_GET['user'])."'");
