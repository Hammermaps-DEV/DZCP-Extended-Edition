...
else
{
    ...
    $show = ''; $color = 1;
    $qry = db("SELECT id,datum,nick,email,text,ip FROM ".dba::get('shout')." ORDER BY datum DESC LIMIT ".($page - 1)*$maxshoutarchiv.",".$maxshoutarchiv."");
    while($get = _fetch($qry))
    {
        ...
        $del = (permission("shoutbox") ? "<a href='?index=shout&amp;action=admin&amp;do=delete&amp;id=".$get['id']."'><img src='inc/images/delete_small.gif' border='0' alt=''></a>" : "");
    ...
    $nav = nav($entrys,$maxshoutarchiv,"?index=shout&amp;action=archiv");
