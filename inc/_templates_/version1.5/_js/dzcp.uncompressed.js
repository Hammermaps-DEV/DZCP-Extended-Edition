...
// DZCP JAVASCRIPT LIBARY FOR JQUERY V2.0.0
  ...
  //init
    init: function() {
      ...
      // refresh teamspeak
      if($('#navTeamspeakContent')[0]) window.setInterval("DZCP.initDynLoader('navTeamspeakServer','teamspeak','');", teamspeakInterval);
  ...
  // handle events
    ...
    initDynLoader: function(tag,menu,options) {
        var request = $.ajax({ url: "../inc/ajax.php?loader=menu&mod=" + menu + options, type: "GET", data: {}, cache:false, dataType: "html" });
        request.done(function(msg) { $('#' + tag).html( msg ).hide().fadeIn("normal"); });
    },
  ...
  // handle info layer
    ...
    ajaxPreview: function(form) {
        ...
        $('#previewDIV').html('<div style="width:100%;text-align:center"><img src="../inc/images/admin/loading.gif" alt="" /></div>');
