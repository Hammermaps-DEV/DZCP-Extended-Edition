...
*
* Modified by Godkiller_NT (Hammermaps.de) for DZCP-Extended Edition 08-04-2013 (mm-dd-yyyy)
*/
(function($) {
    $.fn.imgscale = function(params) {

        params = $.extend( {parent: false, scale: 'fill', center: true, fade: 0, WidthPX: false, HeightPX: false}, params);
        ...
        this.each( function()
        {
            ...
            if( $parent.length > 0 )
            {
                ...
                if (this.complete)
                {
                    _scaleImage( $img, $parent, false, params.WidthPX, params.HeightPX);
                }
                else
                {
                    $img.load( function()
                            {
                        _scaleImage( $img, $parent, true, params.WidthPX, params.HeightPX );
                    });
                }
            }
        });

        function _scaleImage( $img, $parent, _loadedImg, $WidthPX , $HeightPX)
        {
            _parentHeight = !$HeightPX ? $parent.height() : $HeightPX;
            _parentWidth = !$WidthPX ? $parent.width() : $WidthPX;
