// GLOBAL VARS
  ...
  var index = new Array(); var i = 0; var pool = new Pool(6);
...
// DZCP JAVASCRIPT LIBARY FOR JQUERY >= V1.9
  ...
  //init
    init: function()
    {
        ...
        // refresh shoutbox
        if($('#navShout')) window.setInterval("$('#navShout').load('inc/ajax.php?loader=menu&mod=shoutbox');", (json.shout_refresh * 1000)); // refresh interval of the shoutbox in ms
  ...
  // handle events
    ...
    shoutSubmit: function() {
        $.post('?index=shout&ajax', $('#shoutForm').serialize(),function(req)
        {
            var securimage = 'inc/ajax.php?loader=securimage&lines=0&namespace=menu_shout&length=2&width=60&sid=' + Math.random();
            if(req.length >= 5)
            {
                if(json.shout_securimage == '1')
                    $('#siimage_menu_shout').attr('src',securimage);

                alert(req.replace(/  /g, ' ').replace('&uuml;', '\u00fc').replace('&Uuml;', '\u00dc').replace('&auml;', '\u00e4').
                        replace('&Auml;', '\u00c4').replace('&ouml;', '\u00f6').replace('&Ouml;', '\u00d6').replace('&szlig;', '\u00df').replace('ï»¿', ''));
            }

            $('#navShout').load('inc/ajax.php?loader=menu&mod=shoutbox');
