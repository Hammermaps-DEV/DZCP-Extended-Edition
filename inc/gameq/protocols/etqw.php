...
class GameQ_Protocols_Etqw extends GameQ_Protocols
{
    ...
    */
    protected function process_dzcp_runtime()
    {
        ...
        $result->add('game_map', string::decode(str_replace('maps/', '', $this->server_data_stream['map'])));
        ...
        */
        ...
        if(array_key_exists('players', $this->server_data_stream) && count($this->server_data_stream['players']) >= 1)
        {
            foreach($this->server_data_stream['players'] as $player)
            {
                ...
                $new_player['player_team'] = convert::ToString(empty($player['teamname']) ? '0' : $player['teamname']);
                $new_player['player_squad'] = '0';
                $new_player['player_kills'] = convert::ToString(empty($player['total_kills']) ? '0' : $player['total_kills']);
                $new_player['player_deaths'] = convert::ToString(empty($player['total_deaths']) ? '0' : $player['total_deaths']);
                $new_player['player_rank'] = '0';
                $new_player['player_ping'] = convert::ToString(empty($player['ping']) ? '0' : $player['ping']);
                ...
                $teams[convert::ToString(empty($player['teamname']) ? '0' : $player['teamname'])] = array('team_name' => convert::ToString(empty($player['teamname']) ? '0' : $player['teamname']), 'team_score' => 0);
            }
            unset($player_index);
        }
