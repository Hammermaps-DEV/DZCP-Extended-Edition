...
class GameQ_Protocols_Crysiswars extends GameQ_Protocols_Gamespy3
{
    ...
    */
    protected function process_dzcp_runtime()
    {
        ...
        $result->add('game_map', string::decode($this->server_data_stream['mapname']));
        ...
        */
        ...
        if(array_key_exists('players', $this->server_data_stream) && count($this->server_data_stream['players']) >= 1)
        {
            foreach($this->server_data_stream['players'] as $player)
            {
                ...
                $new_player['player_team'] = convert::ToString(empty($player['team']) ? '3' : $player['team']);
                $new_player['player_squad'] = '0';
                $new_player['player_kills'] = convert::ToString(empty($player['kills']) ? '0' : $player['kills']);
                $new_player['player_deaths'] = convert::ToString(empty($player['deaths']) ? '0' : $player['deaths']);
                $new_player['player_rank'] = convert::ToString(empty($player['rank']) ? '0' : $player['rank']);
