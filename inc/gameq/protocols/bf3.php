...
class GameQ_Protocols_Bf3 extends GameQ_Protocols
{
    ...
    */
    protected function process_dzcp_runtime()
    {
        ...
        $result->add('game_map', string::decode($this->server_data_stream['map']));
            ...
            */
            ...
            if(isset($this->server_data_stream['players']) && count($this->server_data_stream['players']) >= 1)
            {
                foreach($this->server_data_stream['players'] as $player)
                {
                    ...
                    $new_player['player_score'] = convert::ToString($player['score']);
                    $new_player['player_time'] = '0';
                    $new_player['player_team'] = convert::ToInt(empty($player['teamId']) ? 0 : $player['teamId']);
                    $new_player['player_squadid'] = convert::ToInt($player['squadId']);
                    $new_player['player_squad'] = convert::ToString($this->squadlist[$player['squadId']]);
                    $new_player['player_kills'] = convert::ToString($player['kills']);
                    $new_player['player_deaths'] = convert::ToString($player['deaths']);
                    $new_player['player_rank'] = convert::ToString($player['rank']);
                ...
                $players_team1 = record_sort($players_team1, 'player_squadid', true); //Sort
                $players_team2 = record_sort($players_team2, 'player_squadid', true); //Sort
                $players_team3 = record_sort($players_team3, 'player_squadid', true); //Sort
            ...
            {
                case 'score': if($this->stats_options['stats_score']) $player_list = record_sort($player_list, 'player_score', true); break;
                case 'time': if($this->stats_options['stats_time']) $player_list = record_sort($player_list, 'player_time', true); break;
                case 'team': if($this->stats_options['stats_team']) $player_list = record_sort($player_list, 'player_team', true); break;
                case 'squad': if($this->stats_options['stats_squad']) $player_list = record_sort($player_list, 'player_squad', true); break;
                case 'kills': if($this->stats_options['stats_kills']) $player_list = record_sort($player_list, 'player_kills', true); break;
                case 'deaths': if($this->stats_options['stats_deaths']) $player_list = record_sort($player_list, 'player_deaths', true); break;
                case 'rank': if($this->stats_options['stats_rank']) $player_list = record_sort($player_list, 'player_rank', true); break;
                case 'ping': if($this->stats_options['stats_ping']) $player_list = record_sort($player_list, 'player_ping', true); break;
                case 'honor': if($this->stats_options['stats_honor']) $player_list = record_sort($player_list, 'player_honor', true); break;
                case 'goal': if($this->stats_options['stats_goal']) $player_list = record_sort($player_list, 'player_goal', true); break;
                case 'leader': if($this->stats_options['stats_leader']) $player_list = record_sort($player_list, 'player_leader', true); break;
                case 'stats': if($this->stats_options['stats_stats']) $player_list = record_sort($player_list, 'player_stats', true); break;
            }
