...
class FTP
{
    ...
    public static function move($dir='')
    {
        ...
        if(self::$connect != false && is_resource(self::$connect) && self::$login)
        {
            ...
            {
                $dirs = explode('/', $dir); $dir_path = '/'; $i=1;
                foreach($dirs as $dir)
                {
                    if(empty($dir)) continue;
                    $contents = ftp_nlist(self::$connect, $dir_path);
                    $dir_path .= ($i == 1 ? $dir : '');
                    $dir_path .= ($i >= 2 ? '/'.$dir : '');

                    if(!array_var_exists($dir_path,$contents) && $dir_path != './')
                    {
            ...
            self::$dir = str_replace('.', '', $dir_path);
            ftp_chdir(self::$connect, self::$dir);
            DebugConsole::insert_info('FTP::move()', 'Go to dir: "'.self::$dir.'"');
            return true;
        }
