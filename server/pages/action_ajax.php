...
else
{
    function server_show($sID = 0, $showID = 0)
    {
        ...
        $cache_hash = md5($html5_worker_sync.$get['ip'].':'.$get['port'].'_'.$get['game']);
        ...
        if(!$static_server)
        {
            if(Cache::check('server_'.$cache_hash))
            {
                ...
                DebugConsole::insert_info('server/pages/action_ajax.php', 'Summon data from host: "'.$get['ip'].':'.$get['port'].'"');
            ...
            {
                $get['ip'] = str_replace(' ', '', $get['ip']);
                DebugConsole::insert_successful('server/pages/action_ajax.php', 'Summon data from DZCP-Cache, Server: "'.$get['ip'].':'.$get['port'].'"');
        ...
        $show_score_td = ''; $show_deaths_td = ''; $show_skill_td = ''; $show_goal_td = ''; $show_honor_td = ''; $show_squad_td = ''; $cteam = '';
        ...
        if(!empty($server) && $server && $server['game_online']) //Online
        {
            ...
            API_EVENTS::server_image_map($server['game_map_pic_dir'].'/'.strtolower(str_ireplace(' ', '_', $server['game_map'])),$server['game_name_long'],strtolower(str_ireplace(' ', '_', $server['game_map'])),!empty($server['game_mod']) ? $icon_mod : $icon_basic); //API Events
