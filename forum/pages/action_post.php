...
else
{
    if($do == "edit")
    {
        $get = db("SELECT * FROM ".dba::get('f_posts')." WHERE id = '".convert::ToInt($_GET['id'])."'",false,true);
    ...
    elseif($_GET['do'] == "add")
    {
        ...
        else
        {
            ...
            {
                $checks = db("SELECT s2.id,s1.intern FROM ".dba::get('f_kats')." AS s1
                             LEFT JOIN ".dba::get('f_skats')." AS s2
                             ON s2.sid = s1.id
                             WHERE s2.id = '".convert::ToInt($_GET['kid'])."'",false,true);

                if(forumcheck(convert::ToInt($_GET['id']), "closed"))
                    $index = error(_error_forum_closed);
                elseif($checks['intern'] && !permission("intforum") && !fintern($checks['id']))
                ...
                else
                {
                    ...
                    if(isset($_GET['zitat']))
                    {
                        $getzitat = db("SELECT nick,reg,text FROM ".dba::get('f_posts')." WHERE id = '".convert::ToInt($_GET['zitat'])."'",false,true);
                        $nick = !$getzitat['reg'] ? $getzitat['nick'] : autor($getzitat['reg']);
                        $zitat = bbcode::zitat($nick, string::decode($getzitat['text']));
                    }
                    else if(isset($_GET['zitat_t']))
                    {
                        $getzitat = db("SELECT t_nick,t_reg,t_text FROM ".dba::get('f_threads')." WHERE id = '".convert::ToInt($_GET['zitat_t'])."'",false,true);
                        $nick = !$getzitat['t_reg'] ? $getzitat['t_nick'] : data($getzitat['t_reg'], "nick");
                        $zitat = bbcode::zitat($nick, string::decode($getzitat['t_text']));
                    }
                    ...
                    if(_rows($qryl))
                    {
                        $getl = _fetch($qryl); //SQL-Fetch
                        $user_signatur = data($getl['reg'], "signatur"); //User Signatur
                        $sig = !empty($user_signatur) ? _sig.bbcode::parse_html(string::decode($user_signatur)) : '';

                        $userposts = $getl['reg'] ? show(_forum_user_posts, array("posts" => userstats($getl['reg'], "forumposts"))) : '';
                        $onoff = $getl['reg'] ? onlinecheck($getl['reg']) : '';
                        $posted_ip = checkme() == "4" ? $getl['ip'] : _logged;
                        ...
                        if($getl['reg'] != 0)
                        {
                            $getu = db("SELECT nick,icq,hp,email FROM ".dba::get('users')." WHERE id = '".$getl['reg']."'",false,true);
                            $email = show(_emailicon_forum, array("email" => eMailAddr($getu['email'])));
                            $pn = _forum_pn_preview; $icq = ""; $hp = "";
                    ...
                    {
                        $gett = db("SELECT * FROM ".dba::get('f_threads')." WHERE kid = '".convert::ToInt($_GET['kid'])."' AND id = '".convert::ToInt($_GET['id'])."'",false,true);
                        $user_signatur = data($gett['t_reg'], "signatur"); //User Signatur
                        $sig = !empty($user_signatur) ? _sig.bbcode::parse_html(string::decode($user_signatur)) : '';

                        $userposts = $gett['t_reg'] ? show(_forum_user_posts, array("posts" => userstats($gett['t_reg'], "forumposts"))) : '';
                        $onoff = $gett['t_reg'] ? onlinecheck($gett['t_reg']) : '';
                        $ftxt = isset($_GET['hl']) ? hl(string::decode($gett['t_text']), isset($_GET['hl'])) : '';
                        $text = isset($_GET['hl']) ? bbcode::parse_html(string::decode($ftxt['text'])) : bbcode::parse_html(string::decode($gett['t_text']));
                        $posted_ip = checkme() == "4" ? $gett['ip'] : _logged;
                        ...
                        if($gett['t_reg'] != 0)
                        {
                            $getu = db("SELECT nick,icq,hp,email FROM ".dba::get('users')." WHERE id = '".$gett['t_reg']."'",false,true);
    ...
    elseif($do == "addpost")
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if($checks['intern'] && !permission("intforum") && !fintern($checks['id']))
                    exit();
                ...
                if($toCheck)
                {
                    ...
                    if(_rows($qryl))
                    {
                        ...
                        $ftxt = hl(string::decode($getl['text']), $_GET['hl']);
                        if($_GET['hl']) $text = bbcode::parse_html(string::decode($ftxt['text']));
                        else $text = bbcode::parse_html(string::decode($getl['text']));
                        ...
                        $ftxt = hl(string::decode($gett['t_text']), $_GET['hl']);
                        if($_GET['hl']) $text = bbcode::parse_html(string::decode($ftxt['text']));
                        else $text = bbcode::parse_html(string::decode($gett['t_text']));
                    ...
                    if($spam == 1)
                    {
                        ...
                        $text = show(_forum_spam_text, array("autor" => $fautor,
                                "ltext" => addslashes(string::decode($getdp['text'])),
                        ...
                        $text = show(_forum_spam_text, array("autor" => $fautor,
                                "ltext" => addslashes(string::decode($gettdp['t_text'])),
                                "ntext" => string::encode($_POST['eintrag'])));

                        $qry = db("UPDATE ".dba::get('f_threads')."
                                                 SET `lp`   = '".time()."',
                                                 `t_text`   = '".string::encode($text)."'
                                                 WHERE id = '".$gettdp['id']."'");
                    } else {
