...
if($count_spam >= 1)
{
    ...
    if($start != 0)
        $pager_sfs = '<a href="?index=admin&amp;admin=ipban&sfs_side='.($site-1).'&sys_side='.(isset($_GET['sys_side']) ? $_GET['sys_side'] : 1).'&ub_side='.(isset($_GET['ub_side']) ? $_GET['ub_side'] : 1).'"><img align="absmiddle" src="inc/images/previous.png" alt="left" /></a>';
    else
    ...
    if($count_spam_nav >= 10 )
        $pager_sfs .=  '<a href="?index=admin&amp;admin=ipban&sfs_side='.($site+1).'&sys_side='.(isset($_GET['sys_side']) ? $_GET['sys_side'] : 1).'&ub_side='.(isset($_GET['ub_side']) ? $_GET['ub_side'] : 1).'"><img align="absmiddle" src="inc/images/next.png" alt="right" /></a>';
    else
    ...
    while($get = _fetch($qry))
    {
        ...
        $delete = show("page/button_delete_single", array("id" => $get['id'], "action" => "index=admin&amp;admin=ipban&amp;do=delete", "title" => _button_title_del, "del" => _confirm_del_ipban));
        $action = "?index=admin&amp;admin=ipban&amp;do=enable&amp;id=".$get['id']."&amp;sfs_side=".($site)."&amp;sys_side=".(isset($_GET['sys_side']) ? $_GET['sys_side'] : 1)."&amp;ub_side=".(isset($_GET['ub_side']) ? $_GET['ub_side'] : 1);
...
if($count_sys >= 1)
{
    ...
    if($start != 0)
        $pager_sys = '<a href="?index=admin&amp;admin=ipban&sys_side='.($site-1).'&sfs_side='.(isset($_GET['sfs_side']) ? $_GET['sfs_side'] : 1).'&ub_side='.(isset($_GET['ub_side']) ? $_GET['ub_side'] : 1).'"><img align="absmiddle" src="inc/images/previous.png" alt="left" /></a>';
    else
    ...
    if($count_sys_nav >= 10 )
        $pager_sys .=  '<a href="?index=admin&amp;admin=ipban&sys_side='.($site+1).'&sfs_side='.(isset($_GET['sfs_side']) ? $_GET['sfs_side'] : 1).'&ub_side='.(isset($_GET['ub_side']) ? $_GET['ub_side'] : 1).'"><img align="absmiddle" src="inc/images/next.png" alt="right" /></a>';
    else
    ...
    while($get = _fetch($qry))
    {
        $data_array = string_to_array(hextobin($get['data']));
        $delete = show("page/button_delete_single", array("id" => $get['id'], "action" => "index=admin&amp;admin=ipban&amp;do=delete", "title" => _button_title_del, "del" => _confirm_del_ipban));
        $action = "?index=admin&amp;admin=ipban&amp;do=enable&amp;id=".$get['id']."&amp;sys_side=".($site)."&amp;sfs_side=".(isset($_GET['sfs_side']) ? $_GET['sfs_side'] : 1)."&amp;ub_side=".(isset($_GET['ub_side']) ? $_GET['ub_side'] : 1);
        $unban = ($get['enable'] ? show(_ipban_menu_icon_enable, array("id" => $get['id'], "action" => $action, "info" => show(_confirm_disable_ipban,array('ip'=>$get['ip'])))) : show(_ipban_menu_icon_disable, array("id" => $get['id'], "action" => $action, "info" => show(_confirm_enable_ipban,array('ip'=>$get['ip'])))));
        $class = ($color % 2) ? "contentMainSecond" : "contentMainFirst"; $color++;
...
if($count_user >= 1)
{
    ...
    if($start != 0)
        $pager_user = '<a href="?index=admin&amp;admin=ipban&ub_side='.($site-1).'&sys_side='.(isset($_GET['sys_side']) ? $_GET['sys_side'] : 1).'&sfs_side='.(isset($_GET['sfs_side']) ? $_GET['sfs_side'] : 1).'"><img align="absmiddle" src="inc/images/previous.png" alt="left" /></a>';
    else
    ...
    if($count_user_nav >= 10 )
        $pager_user .=  '<a href="?index=admin&amp;admin=ipban&ub_side='.($site+1).'&sys_side='.(isset($_GET['sys_side']) ? $_GET['sys_side'] : 1).'&sfs_side='.(isset($_GET['sfs_side']) ? $_GET['sfs_side'] : 1).'"><img align="absmiddle" src="inc/images/next.png" alt="right" /></a>';
    else
    ...
    {
        $data_array = string_to_array(hextobin($get['data']));
        $edit = show("page/button_edit_single", array("id" => $get['id'], "action" => "index=admin&amp;admin=ipban&amp;do=edit", "title" => _button_title_edit));
        $delete = show("page/button_delete_single", array("id" => $get['id'], "action" => "index=admin&amp;admin=ipban&amp;do=delete", "title" => _button_title_del, "del" => _confirm_del_ipban));
        $action = "?index=admin&amp;admin=ipban&amp;do=enable&amp;id=".$get['id']."&amp;ub_side=".($site)."&amp;sys_side=".(isset($_GET['sys_side']) ? $_GET['sys_side'] : 1)."&amp;sfs_side=".(isset($_GET['sfs_side']) ? $_GET['sfs_side'] : 1);
        $unban = ($get['enable'] ? show(_ipban_menu_icon_enable, array("id" => $get['id'], "action" => $action, "info" => show(_confirm_disable_ipban,array('ip'=>$get['ip'])))) : show(_ipban_menu_icon_disable, array("id" => $get['id'], "action" => $action, "info" => show(_confirm_enable_ipban,array('ip'=>$get['ip'])))));
        $class = ($color % 2) ? "contentMainSecond" : "contentMainFirst"; $color++;
