...
while($get = _fetch($qry_search))
{
    ...
    $edit =$get['typ'] == '3' ? show("page/button_edit_single", array("id" => $get['id'], "action" => "index=admin&amp;admin=ipban&amp;do=edit", "title" => _button_title_edit)) : '';
    $action = "?index=admin&amp;admin=ipban&amp;do=enable&amp;id=".$get['id']."&amp;ub_side=".(isset($_GET['ub_side']) ? $_GET['ub_side'] : 1)."&amp;sys_side=".(isset($_GET['sys_side']) ? $_GET['sys_side'] : 1)."&amp;sfs_side=".(isset($_GET['sfs_side']) ? $_GET['sfs_side'] : 1);
